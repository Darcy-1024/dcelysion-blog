...
交付文件与验证结果。若用户尚未授权提交或推送，保留本地结果并说明未提交或未推送；只有完成用户要求的发布目标需要该授权时才询问。已有本任务明确授权则按授权继续，不重复索要相同确认。只暂存任务文件，使用 Conventional Commits，并验证推送及配置的自动部署实际结果。Sites 授权与确认方式以根目录规定为准。
