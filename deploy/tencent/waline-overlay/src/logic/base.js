...
module.exports = class BaseLogic extends think.Logic {
  ...
  async __before() {
    ...
    try {
      session = sessionToken.verify(token, think.config('jwtKey'));
    } catch (err) {
      // Invalid bearer and OAuth state values remain unauthenticated.
    }
    ...
    const user = await this.modelInstance.select(
      { objectId: session.userId, type: ['!=', 'banned'] },
      {
        field: [
          'id',
          'email',
    ...
    const [userInfo] = user;
    if (Number(userInfo.auth_version) !== session.version) {
      return;
    }
