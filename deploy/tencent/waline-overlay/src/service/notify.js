...
const nodemailer = require('../../password-reset/node_modules/nodemailer');
...
module.exports = class NotifyService extends think.Service {
  ...
  async mail({ to, title, content }, self, parent) {
    ...
    title = this.controller.mailLocale(title, data);
    content = this.controller.mailLocale(content, data);
...
仅供评论预览，点击[查看完整內容]({{site.postUrl}})`;
  ...
  async run(comment, parent, disableAuthorNotify = false) {
    ...
    // Automated mail uses audited Simplified Chinese templates regardless of
    // request language or inherited English custom template settings.
    const mailList = [];
    ...
    const title = 'MAIL_SUBJECT_ADMIN';
    const content = 'MAIL_TEMPLATE_ADMIN';
    ...
    for (const mail of mailList) {
      try {
        await this.mail(mail, comment, parent);
        console.log('Comment notification mail accepted by SMTP');
      } catch {
        console.error('Comment notification mail failed');
      }
    }
  }
};
